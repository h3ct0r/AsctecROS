...
namespace asctec
{
  SerialInterface::SerialInterface (std::string port, uint32_t speed):serialport_name_ (port), serialport_speed_ (speed)
  {
    struct termios tio;
      ...
      dev_ = open(serialport_name_.c_str (),O_RDWR | O_NOCTTY | O_NDELAY | O_NONBLOCK );
      ...
      // OLD CODE TO CONNECT TO SERIAL PORT
      /*
      ...
      */
      ...
      cfsetispeed(&tio, B57600);    // set baud rates
      tio.c_cflag = B57600 | CS8 | CREAD | CLOCAL;
      tio.c_iflag = IGNPAR;
      tio.c_oflag = 0;
      tio.c_lflag = 0;
      tcsetattr(dev_, TCSANOW, &tio); // apply the settings to the port
      
      // Extra information for Waypoints 
      waypoint_cmd_updated = true;
  }
  ...
  int SerialInterface::wait (int bytes_requested)
  {
    ...
    while (bytes_available < bytes_requested)
    {
      ...
      //ROS_ERROR("Waiting for serial interface... SerialInterface::wait");
      i++;
    }
    return bytes_available;
    */
                                   ...
                                   unsigned short &packet_size)
  {
    ...
    wait(3);
    ROS_DEBUG ("  SerialInterface::getPacket() pass wait1");
    ...
    if (i == 0 || strncmp (stoken, "<#<", 3) != 0)
    {
      unsigned int controlCounter_= 0;
      ROS_ERROR ("Error Reading Packet Footer: %s", strerror (errno));
      ROS_DEBUG ("Read (%d): %s", i, stoken);
      while (read (dev_, stoken, 1) != 0 && controlCounter_ < 3000) //3000 is a random number to test and exit the while loop...
      {
        stoken[1] = '\0';
        ROS_DEBUG ("%s", stoken);
        controlCounter_++;
...
void SerialInterface::sendWaypointCommands (Telemetry * telemetry) {
    ...
    for(int i = 0; i < sizeof(data)/sizeof(data[0]); i++){
      data[i] = char(32); //char(32) its the space character ' '
    }

    ROS_INFO ("sendWaypointCommands started");

    if(telemetry->WAYPOINT_COMMAND_.cmd.length() < 5){
      ROS_INFO("CMD command < 5 or null");
      return;
    }
    ...
    ROS_INFO("Sending control command %s.", cstr);
    ...
    telemetry->WAYPOINT_COMMAND_.cmd = "";
     // telemetry->WAYPOINT_.wp_number = 0;
    ...
    /*wait(5);
    i = read (dev_,data,5);
    ROS_INFO("Control CMD Response String %s for %s",data, cstr);*/
    ...
    wait(5);
    i = read (dev_,data,5);
    ROS_INFO("Control WP Response String %s",data);

    if (i != 5) {
      ROS_ERROR("Control Response : Insufficient Data, acknowledge < 5 bytes");
      flush();
      return;
    }
    if (strncmp(data,">a",2) != 0) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  bool SerialInterface::getPackets (Telemetry * telemetry)
  {
    ...
    //ROS_DEBUG
    ROS_INFO ("  Requesting %04x %zd packets", (short) telemetry->requestPackets_.to_ulong (), telemetry->requestPackets_.count ());
    // ORIGINAL CODE
    //sprintf (cmd, ">*>p%c", (short) telemetry->requestPackets_.to_ulong ());
    //output (cmd, 6);

    //sprintf (cmd, ">*>p%hu", ((short) telemetry->requestPackets_.to_ulong ()) & 0xff);
    //sprintf (cmd, ">*>p%c", (unsigned short) 0x1000);
    //sprintf (cmd, ">*>p1");
    //ROS_INFO("  Sending cmd request %s", cmd);
    //for (int y = 0; y < 16; y++){
    //  ROS_INFO("0x%x ", cmd[y]);
    //}
    //ROS_INFO("SENDING REQUEST %04x", (short) telemetry->requestPackets_.to_ulong ());
    
    // MODIFIED CODE
    POLL_REQUEST req = { {'>', '*', '>', 'p'}, (short) telemetry->requestPackets_.to_ulong () };
    //ROS_INFO("SIZEOF POLL_REQUEST %u", sizeof(POLL_REQUEST));
    int size = write (dev_, &req, sizeof(POLL_REQUEST));
    fsync(dev_);
    //ROS_INFO("%d bytes written.\n", size);
    if (size == 0)
    {
      ROS_ERROR ("Error 0 bytes written");
      return false;
    }
    ...
    for (i = 0; i < telemetry->requestPackets_.count (); i++)
    {
      ...
      if (read_result)
      {
        ...
        else if (packet_type == Telemetry::PD_CURRENTWAY)
        {
          ...
          {
            ROS_DEBUG ("  CURRENT_WAY Packet CRC is valid");
            result = true;
          }
        }
        else

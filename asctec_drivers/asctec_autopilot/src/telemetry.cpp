...
namespace asctec
{
  ...
  void Telemetry::buildRequest ()
  {
    ...
    {
      //ROS_INFO("number %u getNumSubscribers() %u mask %u", i, requestPublisher_[i].getNumSubscribers(), REQUEST_BITMASK[i]);
      ...
      if (requestInterval_[i] != 0 && ((requestCount_ - requestOffset_[i]) % requestInterval_[i] == 0) &&
        ...
        //ROS_INFO("buildRequest: request packet %lu %zd", requestPackets_.to_ulong(), requestPackets_.count());
      }
    }
  }
  void Telemetry::publishPackets ()
  {
    for (int i = 0; i < REQUEST_TYPES; i++)
    {
      if (requestInterval_[i] != 0 && ((requestCount_ - requestOffset_[i]) % requestInterval_[i] == 0))
      {
        //ROS_INFO("publishPackets: request type %u", i);
        switch (i)
        {
          ...
          case RequestTypes::CURRENT_WAY:
            //ROS_INFO ("Publishing CURRENT_WAY ...");
            copyCURRENT_WAY ();
  ...
  void Telemetry::enablePolling (RequestType msg, uint8_t interval, uint8_t offset)
  {
    switch (msg)
    {
      ...
      case RequestTypes::CONTROLLER_OUTPUT:
        ...
        break;
      case RequestTypes::CURRENT_WAY:
        //ROS_INFO ("enablePolling CURRENT_WAY, BITMASk %u", REQUEST_BITMASK[RequestTypes::CURRENT_WAY]);
...
/////////////////////////////////////
  void Telemetry::copyWAYPOINT_COMMAND (asctec_msgs::WaypointCommand msg){
    ROS_INFO ("copyWAYPOINT_COMMAND %s", msg.cmd.c_str());
